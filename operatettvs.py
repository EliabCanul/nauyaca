...
from .utils import writefile, intervals # Miztli: from NAU.utils import writefile, intervals
import copy
import warnings
...
class Optimizers:
    ...
    def _differential_evolution_nelder_mead(PSystem, base_name, indi=0):
        ...
        DE = differential_evolution( 
                log_likelihood_func, bounds, 
                disp=False, seed=np.random.seed(),
                popsize=100, maxiter=5, polish=False, #int(50/(i+1.)) 150 18
        ...
        if f0 < f0_before:
            ...
            length = [(bounds[j][1] - bounds[j][0])*.2
        ...
        #x0 = []
        #for b in bounds:
        #    x0.append(np.random.uniform(b[0],b[1] ))
        
        PO = minimize(
            log_likelihood_func, list(x0), method= 'Powell', #'Nelder-Mead', 
            options={'maxiter':15000, 'maxfev':15000, 'disp':False, 'adaptive':True}, #15000, 15000
            args=(PSystem,))
        x1 = PO.x
        f1 = PO.fun
        #print(indi+1, 'Pow \t   ', f1)
        ...
        NM = minimize(
            ...
            options={'maxiter':15000, 'maxfev':15000, 'disp':False, 'adaptive':True}, #2000, 1000
            args=(PSystem,))
        x2 = NM.x
        f2 = NM.fun

        print(indi+1, 'DE:', f0, 'PO:', f1, 'NM:', f2)

        """
        length = [(bounds[j][1] - bounds[j][0])*.4
                    for j in range(len(bounds))]
        
        PSystem.bounds = [[max(x1[k]-length[k], original_bounds[k][0]), 
                    min(x1[k]+length[k], original_bounds[k][1])] 
                    for k in range(len(length))]
        ...
        if f1 < f1_before:
            ...
            print(indi+1, 'break it', i, f1_before)
            f1 = f1_before
            x1 = x1_before
            break
        """
        ...
        #if f1 < f1_before and not False in intervals(PSystem.bounds, x1):

        # Insert again constant values
        for k, v in sorted(PSystem.constant_params.items(), key=lambda j:j[0]):
            x2 = np.insert(x2, k, v)

        info = '{} \t {} \n'.format(str(np.round(f2,5)),
                "  ".join([str(np.round(it,5)) for it in x2]))
        #print(indi+1, '\t   ', f1)
        ...
        """
            ...
            print(indi+1, 'else', f1, x1)  # Quitar linea
            # Insert again constant values
            for k, v in sorted(PSystem.constant_params.items(), key=lambda j:j[0]):
                x0 = np.insert(x1, k, v)
...
class MCMC:
    ...
    def run_mcmc(cls, PSystem, 
        ...
        nu = None #100. #/nwalkers 
        t0 = None #1000. #/nwalkers
        a_scale = None#5.
    ...
    @staticmethod
    def restart_mcmc(PSystem, hdf5_file='', Itmax=100, conver_steps=2, cores=1, 
        suffix='_rerun', ntemps=None, Tmax=None, restart_ladder=False):
        #TODO: new hdf5 file must coincide with hdf5_file + suffix
        #TODO: hdf5_file should change to from_hdf5_file??
        print("\n=========== RESTARTING MCMC ===========\n")
        print('Restarting from file: ', hdf5_file)
        ...
        if restart_ladder  == False:
            ladder =  f['BETAS'].value[-1]

        cadena = f['CHAINS'].value[:,:,:index+1,:]
        f.close()
        POP = cadena[:,:,-1,:]
        print('Nwalkers: ',nwalkers)
        print('Ntemps :', len(ladder))
        print('Initial population shape: ', POP.shape)
